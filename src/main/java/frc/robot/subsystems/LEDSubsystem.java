...
public class LEDSubsystem extends SubsystemBase {
    ...
    public LEDSubsystem() {
        ...
        rainbow(LEDSegment.ALL); // sets currentControl + dirty=true
    }

    @Override
    public void periodic() {
        if (dirty) {
            candle.setControl(currentControl);
            dirty = false;
        }
    }
    ...
    private void apply(ControlRequest request) {
        currentControl = request;
        dirty = true;
    }
    ...
    public void setColor(Color color, LEDSegment segment) {
        apply(new SolidColor(getStart(segment), getCount(segment))
    ...
    public void rainbow(LEDSegment segment) {
        ...
        apply(new RainbowAnimation(start, end)
            .withBrightness(1.0)
            .withFrameRate(50)
    ...
    public void strobe(LEDSegment segment) {
        int start = getStart(segment);
        int end   = start + getCount(segment) - 1;
        apply(new StrobeAnimation(start, end)
    ...
    public void rolling(Color color, LEDSegment segment) {
        int start = getStart(segment);
        int end   = start + getCount(segment) - 1;
        apply(new ColorFlowAnimation(start, end)
            .withColor(toRGBW(color))
            .withFrameRate(50)
            .withDirection(AnimationDirectionValue.Forward));
    }
    ...
    public void scanner(Color color, LEDSegment segment) {
        int start = getStart(segment);
        int end   = start + getCount(segment) - 1;
        apply(new LarsonAnimation(start, end)
            .withColor(toRGBW(color))
            .withFrameRate(10)
            .withSize(15)
    ...
    public void rgbFade(LEDSegment segment) {
        int start = getStart(segment);
        int end   = start + getCount(segment) - 1;
        apply(new RgbFadeAnimation(start, end)
            .withFrameRate(50));
    }
    ...
    public void fadeColor(LEDSegment segment, Color color) {
        int start = getStart(segment);
        int end   = start + getCount(segment) - 1;
        apply(new SingleFadeAnimation(start, end)
            .withColor(toRGBW(color))
            .withFrameRate(50));
    }
}